...
export async function generateMetadata({
  ...
  const product = await fetch(
    `https://j4t1lj7y.api.sanity.io/v2023-08-22/data/query/production?query=*[_type == 'products']`
...
async function fetchPreviewData(slug: string) {
  let res = await fetch(
    `https://j4t1lj7y.api.sanity.io/v2023-08-22/data/query/production?query=*%5B_type%3D%3D+%22products%22+%26%26+slug.current+%3D%3D+%22${slug}%22%5D`
  );
  return res.json();
}
